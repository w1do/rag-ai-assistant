...
LLM	OpenAI API (через LLPhant)	Генерация ответов, структурирование голоса, статьи
...
1. Цель проекта
   Разработать MVP сервиса для быстрого создания AI-ассистентов: клиент загружает файл (PDF) или записывает голосовое описание компании — система парсит, векторизует через LLPhant + Qdrant и отвечает на вопросы через RAG-пайплайн. Дополнительная фича — генерация статей по URL конкурента.
...
2. Технологический стек (полный список)
   ...
   База данных	PostgreSQL 15+	Хранение пользователей, ассистентов, чанков, статей
   ...
   LLM	OpenAI API (через LLPhant)	Генерация ответов, структурирование голоса, написание статей
   Эмбеддинги	OpenAI text-embedding-3-small (через LLPhant)	Преобразование текста в векторы (1536 dim)
...
3. Архитектура и взаимодействие сервисов
   ...
   │  │  - GenerateArticlesJob (URL → анализ → статьи)      │   │
   │  └──────────────────────────────────────────────────────┘   │
   │                                                             │
   │  ┌──────────────────────────────────────────────────────┐   │
   │  │  RAG-сервис (LLPhant)                               │   │
...
4. История сохраняется в PostgreSQL
   5.4. Генератор статей по конкурентам
   text
1. Клиент вводит URL конкурента
2. Job: GenerateArticlesJob:
   a) Парсит сайт через Laravel HTTP + DOMDocument
   b) Извлекает текстовый контент
   c) Отправляет в GPT: "Выдели 5 ключевых тем"
   d) По каждой теме генерирует статью (1000-1500 слов)
   e) Сохраняет в таблицу articles
3. Показывает в админке с возможностью редактировать через React
6. Структура БД
   sql
