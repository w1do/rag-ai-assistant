...
### Настройка Nginx (Единый домен botsync.ru)

Система настроена для работы на едином домене с интеллектуальной маршрутизацией между **Laravel** (основное приложение, API, личный кабинет) и **Astro** (лендинг/фронтенд).

**Принцип работы "Любые пути":**
- **Laravel (Мастер)**: Nginx по умолчанию направляет все запросы в Laravel. Это позволяет вам добавлять любые маршруты в `routes/web.php` или `routes/api.php` без необходимости обновлять конфигурацию Nginx.
- **Astro (Fallback)**: Если Laravel возвращает ошибку 404 (путь не найден в приложении), Nginx автоматически перехватывает этот ответ и проксирует запрос в сервис **Astro**. Таким образом, лендинг и любые страницы, созданные в Astro, работают как "запасной" вариант.
- **Статика**: Статические файлы Laravel (Vite, `public/*`) отдаются напрямую Nginx для максимальной производительности.

**Преимущества:**
- Полная свобода в выборе путей для бекенда.
- Автоматическая поддержка лендинга на корневом домене `/`.
- Прозрачная работа API и веб-интерфейса.

**Конфигурация:**
- Конфигурация Nginx находится в `docker/nginx.conf`.
- Используется директива `fastcgi_intercept_errors on` для перехвата 404 ошибок от PHP-FPM.
- Резервный location `@astro` проксирует запросы на порт `4321`.
