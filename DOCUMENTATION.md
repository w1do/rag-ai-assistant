...
- **Главная страница (Astro)**: Запросы к корню `/` направляются напрямую в Astro для максимальной скорости.
- **Laravel (Мастер)**: Для всех остальных путей Nginx по умолчанию пробует Laravel. Это позволяет добавлять любые маршруты в `routes/web.php` или `routes/api.php` без правок конфига.
- **Astro (Fallback)**: Если Laravel возвращает ошибку 404, Nginx перехватывает ее и проксирует оригинальный запрос (`$request_uri`) в **Astro**. Таким образом, любые страницы, созданные в Astro, работают как "запасной" вариант.
- **Статика**: Статические файлы Laravel (Vite, `public/*`) отдаются напрямую Nginx.
