...
Система настроена для работы на едином домене с разделением трафика между **Astro** (фронтенд/лендинг) и **Laravel** (личный кабинет и API).

**Распределение путей:**
- `/` (корень) — Проксируется на сервис **Astro** (порт 4321).
- `/cabinet/*` — Личный кабинет Laravel (Dashboard, профиль, настройки).
- `/apa/*` — Публичный API Laravel.
- `/api/*` — Системный API и сторонние интеграции.
- `/build/*` — Статические ассеты Laravel (Vite).
- `/horizon` — Панель управления очередями.
- `/up` — Health check приложения.

**Конфигурация:**
- Конфигурация Nginx находится в `docker/nginx.conf`.
- Для Astro по умолчанию используется имя сервиса `service-botsync-uxvoqm` и порт `4321`. Если имя в Dokploy изменится, его нужно обновить в конфиге.
